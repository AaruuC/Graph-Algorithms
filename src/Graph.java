...
public class Graph {
    ...
    public Graph(int n) {
        if (n <= 0) {
            throw new IllegalArgumentException("0 or negative input");
        }
        adjList = new ArrayList<HashMap<Integer, Integer>>();
        for (int i = 0; i < n; i++) {
            adjList.add(i, new HashMap<Integer, Integer>());
        }
    }
    ...
    public boolean hasEdge(int u, int v) {
        if (!(u >= 0 && u < adjList.size() && v >= 0 && v < adjList.size())) {
            throw new IllegalArgumentException("vertex does not exist has edge");
        }
    ...
    public int getWeight(int u, int v) {
        if (!(u >= 0 && u < adjList.size() && v >= 0 && v < adjList.size())) {
            throw new IllegalArgumentException("vertex does not exist get weight");
        }
    ...
    public boolean addEdge(int u, int v, int weight) {
        if (!(u >= 0 && u < adjList.size() && v >= 0 && v < adjList.size())) {
            throw new IllegalArgumentException("vertex does not exist adding edge");
        }
        if (u == v) {
            throw new IllegalArgumentException("u is equal to v");
        }
        if (!adjList.get(u).containsKey(v)) {
    ...
    public Set<Integer> outNeighbors(int v) {
        if (!(v >= 0 && v < adjList.size())) {
            throw new IllegalArgumentException("vertex does not exist outneighbors");
        }
